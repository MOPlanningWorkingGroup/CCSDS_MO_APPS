...
public class ComArchiveProvider extends ArchiveInheritanceSkeleton {
	...
	public void retrieve(ObjectType objType, IdentifierList domain, LongList objIds, RetrieveInteraction interaction)
		...
		LOG.log(Level.INFO, "received retrieve request: objType={0}, domain={1}, objIds={2}",
				new Object[] { Dumper.objType(objType), Dumper.names(domain), objIds });
	}
	...
	public void query(Boolean returnBody, ObjectType objType, ArchiveQueryList archiveQuery,
		...
		LOG.log(Level.INFO, "received query request: returnBody={0}, objType={1}, arcQuery={2}, filter={3}",
				new Object[] { returnBody, Dumper.objType(objType), archiveQuery, queryFilter });
	}
	...
	public void count(ObjectType objType, ArchiveQueryList archiveQuery, QueryFilterList queryFilter,
		...
		LOG.log(Level.INFO, "received count request: objType={0}, arcQuery={1}, filter={2}",
				new Object[] { Dumper.objType(objType), archiveQuery, queryFilter });
	}
	...
	public LongList store(Boolean returnIds, ObjectType objType, IdentifierList domain,
			...
			throws MALInteractionException, MALException {
		LOG.log(Level.INFO, "received store request: returnIds={0}, objType={1}, domain={2}, details={3}, bodies={4}",
				new Object[] { returnIds, Dumper.objType(objType), Dumper.names(domain), objDetails, objBodies });
		LongList ids = null;
	...
	public void update(ObjectType objType, IdentifierList domain, ArchiveDetailsList objDetails, ElementList objBodies,
		...
		LOG.log(Level.INFO, "received update request: objType={0}, domain={1}, details={2}, bodies={3}",
				new Object[] { Dumper.objType(objType), Dumper.names(domain), objDetails, objBodies });
	}

	@Override
	public LongList delete(ObjectType objType, IdentifierList domain, LongList objIds, MALInteraction interaction)
			throws MALInteractionException, MALException {
		LOG.log(Level.INFO, "received delete request: objType={0}, domain={1}, objIds={2}",
				new Object[] { Dumper.objType(objType), Dumper.names(domain), objIds });
		return null;
	}
